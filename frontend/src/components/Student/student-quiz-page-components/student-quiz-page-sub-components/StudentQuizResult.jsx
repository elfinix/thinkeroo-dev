...
function StudentQuizResult({ questions, onBack, selectedQuiz }) {
    const [currentView, setCurrentView] = useState("quizResult"); // Manage views if needed
        ...
        return (
            <div key={question.id} className="border-2 p-6 rounded-xl border-primary-3 flex flex-col space-y-4">
                ...
                <div className="flex flex-col space-y-3">
                    {["TF", "MC", "SM"].includes(question.question.type) &&
                        [question.question.choice1, question.question.choice2].map((choiceKey, index) => (
                            <button
                                ...
                                className={`w-full py-2 sm:py-3 rounded-lg text-sm sm:text-base ${
                                    question.studentAnswer.includes(choiceKey)
                    ...
                    {question.type === "IDN" && (
                        <div className="w-full py-2 px-4 rounded-lg border-2 text-text-2 border-primary-3">
