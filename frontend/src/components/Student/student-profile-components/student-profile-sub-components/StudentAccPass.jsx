...
function StudentAccPass({ student }) {
    ...
    const handleConfirmSave = async () => {
        try {
            ...
            const response = await axios.post(`${API_ENDPOINT}/api/users/change_password/`, formData, {
                headers: {
    ...
    const handleDeleteAccount = async () => {
        try {
            const token = localStorage.getItem("token") || sessionStorage.getItem("token");
            await axios.delete(`${API_ENDPOINT}/api/users/delete_account/`, {
                headers: {
                    Authorization: `Token ${token}`,
                },
            });
            console.log("Account deleted successfully");
            // Add any additional logic for account deletion, such as redirecting to a different page
        } catch (error) {
                    ...
                    <div className="relative">
                        <input
                            ...
                            className={`w-full p-2 rounded-lg text-text-1 text-sm bg-primary-1 border border-primary-3 focus:outline-none focus:ring-2 
                                ${
                                    errors.currentPassword || errors.general
                ...
                <div>
                    ...
                    <div className="relative">
                        <input
                            ...
                            className={`w-full p-2 rounded-lg text-text-1 text-sm bg-primary-1 border border-primary-3 focus:outline-none focus:ring-2 
                                ${
                                    errors.newPassword || errors.general
                                        ? "focus:ring-red-500 focus:ring-0 border-red-500"
                                        : "focus:ring-secondary-1"
                                }`}
