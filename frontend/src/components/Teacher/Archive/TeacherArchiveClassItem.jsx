...
const TeacherArchiveClassItem = ({ classData, onUnarchive, onDelete, studentCount }) => {
    const [showPopUp, setShowPopUp] = useState(false);

    return (
        <div className="w-[259px] h-[323px] border-primary-3 border-solid border-2 rounded-[10px] overflow-hidden">
            <div className="teacher_class_card_image w-full h-[150px]">
                <img
                    ...
                    src={classData.banner_img || "https://placehold.co/200x80"}
                    alt={classData.name}
                />
            </div>
            <div className="p-3 px-4 h-[173px] flex flex-col justify-between">
                <div className="flex justify-between items-center relative">
                    <h2 className="text-[20px] font-bold text-text-1 w-[80%]">{classData.name}</h2>
                ...
                <div className="flex items-center justify-between">
                    ...
                    <p className="underline text-text-2">{classData.class_code}</p>
                </div>
            </div>
        </div>
    );
